...
def generate_story_options(request: models.AdventureSetupRequest) -> models.AdventureSetupResponse:
    ...
    try:
        ...
        logger.info(f"LLM Raw Response (Premises): {raw_text}")
        ...
        if isinstance(raw_json, list):
             raw_list = raw_json
        elif isinstance(raw_json, dict) and "premises" in raw_json:
             raw_list = raw_json["premises"]
        else:
             raise ValueError("Unexpected LLM response format for Premises")

        for item in raw_list:
            try:
                # Ensure each premise has the required fields
                if not isinstance(item, dict): continue
                for field in ["id", "title", "hero", "setting", "catalyst"]:
                    if field not in item: item[field] = "..."
                premises.append(models.StoryPremise(**item))
            except Exception as pe:
                logger.warning(f"Skipping malformed premise: {pe}")

        if not premises:
            raise ValueError("No valid premises were generated by the LLM")

        return models.AdventureSetupResponse(premises=premises, selected_vocabulary=words)
    except Exception as e:
...
def generate_story_dag(request: models.GenerateDAGRequest) -> models.StoryDAG:
    ...
    try:
        ...
        raw_text = response.text
        logger.info(f"LLM Raw Response (DAG): {raw_text}")
